...
- テスト作成や全件実行を作業の主役にしない。ベストエフォートは手厚く流すことではなく、今壊しそうな箇所を最小時間で確認して実装を前へ進めること。
...
- 変更直後は対象に直結する最小コマンドを優先する。全件テスト・build・ブラウザ確認は、変更の危険度や節目に見合う場合だけ実行する。
...
## 運用改善
- 同じ種類の問題や同じ指摘が繰り返された場合は、個別対応だけで済ませず、AGENTS.md へ追加すべき恒久ルールを簡潔に提案する。
