...
EditResult<GenerateBundleFromPathResult> CoreState::continue_backbone_generation_from_committed_chain(
  ...
  const BackboneSpec& request = request_plan.request;
  const std::vector<BackboneBundlePlan>& bundle_plans = request_plan.bundle_plans;
  const std::vector<BackboneBundlePlan>& active_bundle_plans = request_plan.active_bundle_plans;
  const std::vector<Vec3d>& guide_points = request_plan.guide_points;
  ...
  BackboneResult generation_backbone = topology_plan.generation_backbone;
  const BackboneResult& existing_backbone = topology_plan.existing_network_backbone;
  const auto& existing_primary_neighbor_by_node = topology_plan.existing_primary_neighbor_by_node;
  const auto& existing_prioritized_session_by_node = topology_plan.existing_prioritized_session_by_node;
  const auto& existing_incident_session_by_node = topology_plan.existing_incident_session_by_node;
  ...
  route_neighbors_by_node.reserve(topology_plan.route_neighbors_by_node.size());
  for (const auto& [node_id, neighbors] : topology_plan.route_neighbors_by_node) {
    std::vector<ObjectId>& remapped_neighbors = route_neighbors_by_node[remap_node_id(node_id)];
  ...
  auto existing_node_position_by_id = topology_plan.existing_node_position_by_id;
  for (const auto& [node_id, node] : support_node_by_id) {
    existing_node_position_by_id[node_id] = node.position;
  }
  const auto& existing_adjacency = topology_plan.existing_adjacency;
  auto existing_junction_by_node = topology_plan.existing_junction_by_node;
  auto active_junction_by_node = topology_plan.active_junction_by_node;
  for (BackboneEdge& edge : generation_backbone.edges) {
  ...
  generation_backbone.edges.erase(std::unique(generation_backbone.edges.begin(), generation_backbone.edges.end(),
                                              [](const BackboneEdge& lhs, const BackboneEdge& rhs) {
                                                return lhs.node_a == rhs.node_a && lhs.node_b == rhs.node_b;
                                              }),
                                  generation_backbone.edges.end());
  for (SupportNode& node : generation_backbone.nodes) {
  ...
  for (JunctionInfo& junction : generation_backbone.junctions) {
  ...
  existing_junction_by_node.clear();
  existing_junction_by_node.reserve(topology_plan.existing_network_backbone.junctions.size());
  ...
  const auto previous_pole_orientation_debug_records = orientation_plan.previous_debug_records;
  ...
  BackbonePoleOrientationPolicyInput pole_orientation_policy_input{
      ...
      previous_pole_orientation_debug_records,
      ...
      [&](ObjectId pole_id, const Pole& old_pole, const PortLayoutYawOverride* previous_override) {
        refresh_owned_endpoints_from_pole(pole_id, &result.change_set, &old_pole, previous_override);
      },
      [&](ObjectId pole_id, const Pole& old_pole) { finalize_pole_transform_update(pole_id, old_pole, &result.change_set); }};
  ...
  BackboneDecisionPhaseOutput decision_phase =
      remap_backbone_decision_phase(topology_plan.decision_phase, remap_node_id);
    ...
    for (const BackboneBundlePlan& plan : active_bundle_plans) {
      ...
      for (const auto& [node_id, relation] : plan_junction_relations_by_node) {
        ...
        if (!merged.is_cross_like) {
          (void)assign_nonroute_pair_for_single_route_junction(&merged, current_support_position);
        }
      }
    }
