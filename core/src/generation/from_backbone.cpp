...
SpanSupportLayoutDecisionSeed build_seed_layout_record(
    ...
    std::uint64_t variation_flow_key, SpanLoweringKind lowering_kind, const Port& port_a, const Port& port_b,
...
CoreState::GenerateFromBackboneSpec(const BackboneSpec& spec) {
  ...
  BackbonePoleOrientationPolicyInput pole_orientation_policy_input{
      ...
      [&](ObjectId node_id, const Vec3d& previous_support_axis, PoleOrientationDebugRecord* debug) -> Vec3d {
        return choose_support_axis_for_layout(node_id, current_support_position(node_id), previous_support_axis, debug);
      },
