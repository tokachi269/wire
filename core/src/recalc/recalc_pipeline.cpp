...
const LoweredSupportGroupPlacement* find_lowered_support_group(const CacheState& cache_state,
  ...
  const auto it =
      cache_state.support_layout_cache.lowered_support_groups.find(lowered_support_group_key_for_endpoint(endpoint));
...
bool CoreState::rebuild_span_visual(ObjectId span_id, std::string* error_message) {
  ...
  auto append_parts_for_port = [&](const Port& port, const SupportLayoutEndpoint* layout_endpoint) {
    ...
    const bool uses_grouped_lowered_support =
        layout_endpoint != nullptr && endpoint_uses_grouped_lowered_support(layout_endpoint);
