...
const SupportNode* CoreView::pending_support_node(ObjectId node_id) const {
  const auto it = std::find_if(state_.debug_.last_generation_support_nodes.begin(),
                               state_.debug_.last_generation_support_nodes.end(),
                               [&](const SupportNode& node) { return node.node_id == node_id; });
  return it == state_.debug_.last_generation_support_nodes.end() ? nullptr : &*it;
}
