...
bool C740_visual_curve_part_stats_count_full_curve_builds() {
  ...
  const std::size_t support_count = static_cast<std::size_t>(std::count_if(
      cache.parts.begin(), cache.parts.end(), [](const wire::core::VisualCurvePart& part) {
...
bool C767_default_hv_emits_one_support_path_per_phase_span() {
  ...
  wire::core::BackboneSpec request = poly3_req(state);
  request.bundles.clear();
  ...
  auto support_cable_it = state.view().cable_templates().find(
      wire::core::kDefaultSupportWireCableTemplateId);
  if (support_cable_it == state.view().cable_templates().end() ||
      support_cable_it->second.name != "SUPPORT_WIRE") {
    return false;
  }
  const wire::core::CableTemplate support_cable = support_cable_it->second;
  ...
  for (const wire::core::VisualCurvePart& part : state.view().visual_curve_parts().parts) {
    ...
    const wire::core::SpanLayoutEntry* layout = state.span_layout(part.source_span_id).entry;
    ...
    const auto primary = wire::core::generation::backbone::make_primary_curve_between(
    ...
    for (std::size_t index = 0; index < part.samples.size(); ++index) {
      const double separation =
          wire::core::Length(part.samples[index] - primary.value.sample_points[index]);
  ...
  wire::core::CableTemplate edited_support_cable = support_cable;
  ...
  wire::core::BackboneSpec baseline_request = poly3_req(without_support);
  ...
  wire::core::CoreState lv_state;
  const wire::core::BundleTemplate& lv = lv_state.view().bundle_templates().at(
  ...
  wire::core::BackboneSpec lv_request = poly3_req(lv_state);
  lv_request.bundles.clear();
  add_backbone_bundle(lv_request, wire::core::BundleKind::kLowVoltage);
  const auto lv_generated = lv_state.GenerateFromBackboneSpec(lv_request);
  if (!lv_generated.ok) return false;
  std::unordered_set<wire::core::ObjectId> lv_support_span_ids{};
  for (const wire::core::VisualCurvePart& part : lv_state.view().visual_curve_parts().parts) {
    if (part.supplemental_kind != wire::core::VisualSupplementalKind::kSupportPath) continue;
    const wire::core::SpanLayoutEntry* layout = lv_state.span_layout(part.source_span_id).entry;
...
bool C695_cable_run_id_is_deterministic() {
  ...
  const auto collect = [](const wire::core::CoreState& state, std::vector<std::string>* out) {
    for (const wire::core::VisualCurvePart& part : state.view().visual_curve_parts().parts) {
      if (part.kind == wire::core::VisualCurvePartKind::kSupplemental) {
        continue;
      }
...
bool C766_row_fixture_and_wire_follow_port_band_lateral_change() {
  constexpr wire::core::ModelAssemblyTemplateId kRowAssembly = 9120;
  constexpr wire::core::ModelAssemblyTemplateId kEndpointAssembly = 9121;
  ...
  std::unordered_map<wire::core::ObjectId, wire::core::Vec3d> port_positions{};
  for (wire::core::ObjectId span_id : generated.value.generated_span_ids) {
    const wire::core::Span* span = state.view().spans().find(span_id);
    if (span == nullptr) return false;
  ...
  for (wire::core::PortPlacementBand& band : pole_type.port_bands) {
    if (band.category != wire::core::ConnectionCategory::kHighVoltage) continue;
  ...
  for (const auto& [port_id, before_position] : port_positions) {
    ...
    if (!almost_equal(port->world_position - before_position,
                      wire::core::ScaleVec(frame.lateral, kDelta), 1e-9)) {
      return false;
    }
  }
  ...
  for (const wire::core::VisualModelInstance& instance :
    ...
    if (instance.stable_key.rfind("row:", 0) == 0) {
      pole_id = static_cast<wire::core::ObjectId>(
          std::stoull(instance.stable_key.substr(pole_id_begin, pole_id_end - pole_id_begin)));
    } else if (instance.stable_key.rfind("port:", 0) == 0) {
      const wire::core::ObjectId port_id = static_cast<wire::core::ObjectId>(
          std::stoull(instance.stable_key.substr(pole_id_begin, pole_id_end - pole_id_begin)));
      const wire::core::Port* port = state.view().ports().find(port_id);
      pole_id = port == nullptr ? wire::core::kInvalidObjectId : port->owner_pole_id;
    }
    const wire::core::Pole* pole = state.view().poles().find(pole_id);
    if (pole == nullptr) return false;
    const wire::core::SavedBackbonePortBinding* pole_binding = nullptr;
    ...
    const wire::core::PoleFrame frame =
        wire::core::BuildPoleFrame(pole->world_transform, pole_binding->layout_yaw_deg);
    const wire::core::Vec3d displacement = instance.world_transform.position - before->second;
    if (!almost_equal(displacement, wire::core::ScaleVec(frame.lateral, kDelta), 1e-9)) {
      return false;
    }
  }
...
bool C769_bundle_placements_duplicate_template_as_independent_bundles() {
  ...
  wire::core::BackboneInputSpec::NodeSpec source_node{};
  ...
  const std::array<wire::core::ObjectId, 2> expected_source_bundles{bundle_a_id, bundle_b_id};
  for (std::size_t index = 0; index < branch_out.value.bundle_ids.size(); ++index) {
    ...
    const wire::core::SpanLayoutView layout = state.span_layout(*span_it);
    ...
    const wire::core::SavedBackboneEdgeBundle* source_edge_bundle =
...
bool C770_backbone_bundle_placement_update_preserves_cross_row_height() {
  constexpr wire::core::ModelAssemblyTemplateId kRowAssembly = 9130;
  constexpr wire::core::ModelAssemblyTemplateId kEndpointAssembly = 9131;
  ...
  const wire::core::ObjectId pole_id = first.value.generated_pole_ids[1];
  const wire::core::Pole* pole = state.view().poles().find(pole_id);
  if (pole == nullptr) return false;
  ...
  const wire::core::ObjectId bundle_id = second.value.bundle_ids.front();
  ...
  auto average_port_height = [&]() -> std::optional<double> {
    ...
    for (const wire::core::SavedBackbonePortBinding& binding : state.view().backbone().port_bindings) {
      ...
      const wire::core::PoleFrame frame =
          wire::core::BuildPoleFrame(owner->world_transform, binding.layout_yaw_deg);
      sum += wire::core::WorldPointToLocal(frame, port->world_position).z;
      ++count;
    }
  ...
  auto average_base_height = [&]() -> std::optional<double> {
    ...
    for (const wire::core::SavedBackbonePortBinding& binding : state.view().backbone().port_bindings) {
      ...
      const wire::core::PortPlacementBand* band = nullptr;
      if (pole_type != nullptr) {
        for (const wire::core::PortPlacementBand& candidate : pole_type->port_bands) {
  ...
  auto row_model_height = [&]() -> std::optional<double> {
    ...
    for (const wire::core::VisualModelInstance& instance : state.view().visual_model_instances().instances) {
      ...
      const wire::core::Pole* current_pole = state.view().poles().find(pole_id);
      if (current_pole == nullptr) return std::nullopt;
      const wire::core::PoleFrame frame =
          wire::core::BuildPoleFrame(current_pole->world_transform, 0.0);
      sum += wire::core::WorldPointToLocal(frame, instance.world_transform.position).z;
      ++count;
    }
    if (count == 0) return std::nullopt;
    return sum / static_cast<double>(count);
