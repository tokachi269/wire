...
wire::core::generation::backbone::PhysicalLinePopulationInput population_input(
...
bool same_instances(const std::vector<wire::core::PhysicalLineInstance>& lhs,
                    const std::vector<wire::core::PhysicalLineInstance>& rhs) {
...
wire::core::ExperimentalLinePopulationConfig lv_population_config() {
...
bool C648_experimental_population_same_seed_is_stable() {
  ...
  const auto first = wire::core::generation::backbone::populate_physical_lines(input);
  const auto second = wire::core::generation::backbone::populate_physical_lines(input);
  return first.ok && second.ok &&
         first.value.diagnostic.extra_count_requested == second.value.diagnostic.extra_count_requested &&
         same_instances(first.value.instances, second.value.instances);
}

bool C649_experimental_population_span_identity_changes_placement() {
  const auto first = wire::core::generation::backbone::populate_physical_lines(population_input(101));
  const auto second = wire::core::generation::backbone::populate_physical_lines(population_input(102));
  if (!first.ok || !second.ok || first.value.instances.empty() || second.value.instances.empty()) {
    return false;
  }
  return dist2(first.value.instances.front().endpoint_a, second.value.instances.front().endpoint_a) > 1e-12;
}
...
bool C650_experimental_population_reserve_blocks_candidates() {
  ...
  const auto result = wire::core::generation::backbone::populate_physical_lines(input);
  return result.ok && result.value.instances.empty() && result.value.diagnostic.extra_count_requested > 0 &&
...
bool C651_experimental_population_spacing_rejects_overlap() {
  ...
  const auto result = wire::core::generation::backbone::populate_physical_lines(input);
  return result.ok && result.value.instances.empty() && result.value.diagnostic.omitted_count == 1;
}
...
bool C652_experimental_population_endpoint_failure_omits_pair() {
  ...
  const auto result = wire::core::generation::backbone::populate_physical_lines(input);
  return result.ok && result.value.instances.empty() &&
         result.value.diagnostic.omitted_count == result.value.diagnostic.extra_count_requested &&
...
bool C653_experimental_population_rejects_duplicate_band_identity() {
  ...
  const auto configured = state.UpdateExperimentalLinePopulationConfig(lv_population_config());
  ...
  const bool has_extra_visual = std::any_of(
      ...
      [](const wire::core::VisualCurvePart& part) {
        return part.kind == wire::core::VisualCurvePartKind::kExperimentalPhysicalLine;
      });
  const bool diagnosed = std::any_of(
      ...
      [](const wire::core::PhysicalLinePopulationDiagnostic& diagnostic) {
...
bool C654_experimental_population_does_not_mutate_logical_topology() {
  ...
  const auto configured = experimental.UpdateExperimentalLinePopulationConfig(lv_population_config());
  if (!configured.ok) {
    return false;
  }
