...
| C185 | HV3 DrawPath terminal は template slot を使う | 2点直線 + HV_3PH bundle | GenerateFromBackboneSpec | Invariant: 開始/終了 pole で実際に lane assignment に使われる HV ports は 3 本とも distinct な template slot (`source_slot_id >= 0`) を使う | SegmentLaneAssignment / Port.source_slot_id | endpoint pole で grouped lane が template slot ではなく generated/existing category port を掴み、三相が一点へ寄る回帰防止 |
