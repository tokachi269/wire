...
| C193 | HV3 branch support は bundle 全体を同一高さで下げる | 既存main + HV branch | GenerateFromBackboneSpec(HV) | Invariant: branch support の 3 lane は bundle 内で同一の lowered height を保ち、row は branch に直交し、自動 down offset は現行 policy 値と一致する | branch port z ordering / row span / last_lane_assignments.branch_down_offset_m | HV3 でも 2高1低にして bundle 内高さを割る回帰や、policy 値を変えても lane assignment に反映されない回帰防止 |
| C195 | Branch support は分岐根だけに留める | 既存main + 2-segment HV branch | GenerateFromBackboneSpec(HV) | Invariant: branch support/down-offset は shared root pole にだけ付き、次の branch pole は通常高さの直交 row へ戻る | last_lane_assignments / downstream branch port z / placement_source | branch run 全体を一段下げてしまい、分岐後の最初の pole まで 2段構成を引きずる回帰防止 |
| C196 | Branch support visual は branch に直交する | 既存main + HV branch | GenerateFromBackboneSpec(HV) + Commit(recalc) | Invariant: branch support visual の support axis は branch chord に直交し、hanger は attachment の真下へ落ちる | SpanVisualCacheEntry.branch_supports | branch support visual が root pole radial を引きずって約45度に寝る回帰防止 |
| C197 | DefaultSingle branch は自動高さ変更しない | 既存main + LV branch | GenerateFromBackboneSpec(LV) + Commit(recalc) | Invariant: branch flow は維持するが branch support/down-offset は自動では入らない | last_lane_assignments / SpanInspectionView.uses_branch_support | DefaultSingle まで category 自動 branch down offset で一段下がる回帰防止 |
