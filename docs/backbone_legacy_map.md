...
| C132/C152-C154/C278/C306-C307 render and style recalc | render distance attributes、variation/style wobble、support-layout composite curve を `Commit` 後の v1 caches で確認 | C/D | 退役済み。C132 の距離属性は bb2 draw acceptance で維持。旧 variation/supplemental/composite materialization は supported scope 外で、recalc cache shape は移植しない |
| C179/C180/C285/C316/C319/C326 backbone recalc inspection | branch curve、main/branch trace、same-level through pair、HV lane curve を v1 inspection と `Commit` 後 cache で確認 | C/D | 退役済み。bb2 saved graph、pair/open/row、layout/geom/draw、viewer branch/cross scenarios が現行制約を担う。旧 relation/trace/recalc polyline shape は移植しない |
| retired capture scene helpers | 旧 support-orientation / lowered-branch / replay capture tests 用に scene を作り `Commit` していた7 helper | A | 呼び出し元退役後に物理削除。代替 wrapper は作らず、viewer representative scenes と bb2 scenario tests を正とする |
...
| recalc pipeline | 残存 | v1 state updates / validation / old tests。viewer controls、cached stats、pole yaw override caller は削除済み | bb2 normal path と supported post-edit から完全に外れた後に削る |
