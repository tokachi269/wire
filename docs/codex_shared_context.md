...
- DrawPath点は入力ガイドであり既定で強制Manualにしない（既定では端点も含めPinしない。Pinは明示オプション）。
- 局所再生成はセッション単位で行い、Manual Pole/Manual Portを保持したままAuto部分のみ更新する。
- 鋭角コーナー（コーナー内角 `theta <= 75°`）では、Port列のside軸を二等分線`b`に直交する向きへ補正し、符号は「内角側に詰めない」方を決定的に選ぶ。
- デバッグで `theta / b / side_dir` を確認できる状態を維持する。

## 16.1 区切りの宣言（今回スコープ）
- 道路上に生成する本格統合（道路エッジ参照、PoleAnchor、道路幅/歩道、入口回避）は応用段階として今回の範囲外。
- 現段階の成立条件は、DrawPath/Guide入力で次の5点が成立していること。
- WireGroup前提生成
- Port手直し保護
- Poleピン留め（Manual/Auto）
- 局所再生成（Manual保持、Autoのみ更新）
- Backbone（Pole間）ルート探索
