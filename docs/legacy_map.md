...
| `UpdateBundleTemplate` | fixed count増減と classifier の`kTopology` policy差分は統一 regenerate で route-local scopeをreconcileし、row key/lane一致bindingを再利用、不一致bindingを退役+再生成する。range化(`count_rule` fixed/range、min/max/default)は全既存bundleの`conductor_count`が新policy内なら出力を再生成せず受理し、外ならmutation前に拒否する。multi-bundle は saved edge_bundles 順で group offset を再構成し、pair row は saved route/order から再確定する。存続spanのattachmentは保持し、退役spanのuser attachmentは拒否する。`population_rules` 差分はtemplate-owned derived outputとして`kReshape`で反映する | post-edit API | fresh生成が拒否する新policy、またはnon-backbone spanを含むscopeを扱うscenarioが必要になった時だけ拡張する |
