...
| `UpdateBundleTemplate` | `Apply Bundle Template` | topology級差分 + 既存bundleあり | fixed count増加はSavedGraph identityを維持して下流再生成する。count減少、range化、policy変更などは引き続きregenerate保留。visual-only/detail差分は現状も通る |
