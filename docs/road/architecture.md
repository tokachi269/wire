...
ADD LANEの変化開始と3車線完成位置は同一corridor上の`SegmentPosition`としてoperationへ渡す。Coreが各位置をcorridor累積距離へ解決し、開始から完成への向きをoperation方向にする。taperは1 segment内だけに作り、segment境界を跨ぐtaperのためにpartial templateを多数生成しない。完成断面はoperation方向のcorridor終端まで維持する。ユーザーへ追加の終端入力を要求せず、開始・完成位置を作るためだけにもsegmentを
分割せず、追加の終端入力も持たない。物理距離やcorridor-globalなLaneAddition entityを保存しない。boundary anchorで決まった横方向原点はoperation方向のcorridor終端までの後続segmentへ派生伝播し、
...
明示topologyを要求する。junctionでは各approachの断面を独立して
解決し、未指定laneはgateで終了する。AddLaneは3/4-way junctionの接続先を横順やlane数から推測しない。1件のLaneConnectionを全断面の解決済み判定には使わない。
道路中心や最寄りlaneから接続先を補わない。
新segmentをtargetにする分岐と、新segmentをsourceにする合流は同じoperation契約を使う。
`resolve_connections`はLaneConnectionからapproach全体のauto lateral shiftを導出しない。laneごとに必要な横方向差は`DerivedLanePath`で処理し、approach lateral shiftはmanual overrideだけが変更できる。
