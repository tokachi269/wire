...
- Webのjunction corner radiusは新規道路の作成値であり、既存道路を再編集しない。新規segmentへ保存し、splitは元segment、corridor extensionは終端segmentから継承する。Add Laneは変更しない。junctionでは隣接approach pairごとに小さい保存値を丸みの上限として採用し、作成順・平均・connection全体のfallback値を使わない。0m pairはsharp connectionとしてsetback由来のBezier controlを追加しない。
