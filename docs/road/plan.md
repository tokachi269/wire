...
**この文書は要求範囲と決定の記録であり、現行契約の正本ではない。**
現在の対応範囲は `supported_operations.md`、authority と生成の流れは `architecture.md`、
公開operationの意味は `operation_semantics.md`、標準機能から外した項目は `backlog.md` を見る。
本文と実装が食い違う場合は正本4文書を優先する。
