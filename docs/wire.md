...
###5.3.5鋭角時のPole向き補正
-鋭角コーナー時は、線間距離が詰まらないことを目的にPole向きとスロット展開を補正できること。
-現時点の固定条件は `corner interior angle < 75°` とし、閾値は定数扱いでUI実行時調整対象にしないこと。
-カテゴリ例外は設けず同一ルールを適用し、不足時は警告止まりにせず自動で広げること。
...
-DrawPath/Guide生成は「頂点=強制Manual」を既定にしない（既定では端点も含めPinしない。Pinは明示オプション）
-PoleのPin/Unpin(PlacementMode)と、セッション単位の局所再生成(Manual保持・Auto更新)を優先実装する
-道路上への本格統合（道路エッジ参照、PoleAnchor、道路幅/歩道、入口回避）はこの段階の範囲外とする
-現段階の成立条件は「Guide入力でのWireGroup生成、Port保護、Poleピン留め、局所再生成、Backbone探索」とする
...
-要件が曖昧な場合は、実装前に不明点を質問して確定してから着手すること。
-表面的な挙動ではなく、ユーザ価値を合格基準として先に明文化すること。
-チャットで確定した方針は、関連ドキュメント(`docs/wire.md`/`docs/codex_shared_context.md`)へ同一ターンで反映すること。
...
新規チャット開始時の運用漏れ防止チェックは `docs/chat_handoff_checklist.md` を参照する。
...
-道路上に生成する本格統合（道路エッジ参照、PoleAnchor、道路幅/歩道、入口回避）は応用段階として今回の範囲外。
-現段階のゴールは、DrawPath/Guide入力だけで成立する次の5点とする。
-WireGroup前提生成
-Portの手直し保護
-Poleのピン留め(Manual/Auto)
-局所再生成(Manual保持、Autoのみ更新)
-Backbone(Pole間)でのルート探索
-鋭角コーナー(コーナー内角`theta<=75°`)でのPort列向き補正（二等分線`b`直交＋内角側に詰めない符号選択）

##12.新規チャット開始時の引き継ぎ必須チェック
新規チャット開始時に仕様漏れを防ぐため、以下を毎回確認すること。

-最新の確定事項は `docs/wire.md` と `docs/codex_shared_context.md` を正として読むこと。
-チャットで新規に確定した方針は、そのターンで `docs` に追記してから作業継続すること。
-鋭角対策は固定ルール（`corner interior angle < 75°`、カテゴリ例外なし、UI調整なし、不足時は自動拡幅）で運用すること。
-既存挙動を壊さないことを優先し、曖昧要件は実装前に質問で確定すること。
-テストは公開API観測のみで書き、実装依存の期待値を入れないこと。
-UI変更時は「作業領域を狭めない」「ウィンドウサイズ/状態の再起動保持」を確認すること。
-生成・再生成の変更時は `wire_core_tests` の全件パスを確認すること。
