...
using internal::sort_unique_stations;
...
// Stations that carry meaning: endpoints, span boundaries, transition limits
// and the stations manual markings need.
Result<bool> append_semantic_stations(const SavedRoadGraph &graph,
                                      const RoadSegment &segment,
                                      DerivedSegment &derived) {
  const double length = derived.length_m;
  derived.semantic_stations_m = {0.0, length};
  ...
  for (std::size_t index = 0; index + 1 < derived.alignment.spans.size();
    ...
    derived.semantic_stations_m.push_back(span_boundary);
  }
  ...
  if (segment.transition.has_value()) {
    ...
    append_if_in_range(derived.semantic_stations_m,
                       station_value(transition->start, length), length);
    append_if_in_range(derived.semantic_stations_m,
                       station_value(transition->end, length), length);
  }
  ...
  for (const ManualLineMarking &marking : graph.manual_lines) {
    ...
    for (std::size_t index = 0; index < points.size(); ++index) {
      ...
      append_if_in_range(derived.semantic_stations_m, point.x, length);
...
Result<bool> derive_segment_sections(const SavedRoadGraph &graph,
  ...
  for (DerivedSegment &derived : segments) {
    ...
    const Result<bool> semantic =
        append_semantic_stations(graph, *segment, derived);
    if (!semantic.ok)
      return semantic;
    ...
    for (const ResolvedConnection &connection : connections) {
      for (const ResolvedApproach &approach : connection.approaches) {
        ...
        if (approach.key.endpoint_role == EndpointRole::kStart) {
          surface_start = std::max(surface_start, approach.gate_station_m);
        } else {
          surface_end = std::min(surface_end, approach.gate_station_m);
        }
      }
    }
    ...
    derived.semantic_stations_m.push_back(surface_start);
    derived.semantic_stations_m.push_back(surface_end);
    sort_unique_stations(derived.semantic_stations_m);

    derived.surface_stations_m.clear();
    for (const double station : derived.semantic_stations_m) {
      if (station >= surface_start - station_epsilon &&
          station <= surface_end + station_epsilon) {
        derived.surface_stations_m.push_back(station);
      }
    }
    ...
    for (int i = 0; i <= intervals; ++i) {
      derived.surface_stations_m.push_back(
          surface_start + surface_length * static_cast<double>(i) / intervals);
    }
    sort_unique_stations(derived.surface_stations_m);

    std::vector<double> stations = derived.semantic_stations_m;
    stations.insert(stations.end(), derived.surface_stations_m.begin(),
                    derived.surface_stations_m.end());
    sort_unique_stations(stations);
    derived.sections.clear();
    derived.sections.reserve(stations.size());
    for (const double station : stations) {
      Result<SectionEvaluation> evaluated =
          internal::section_at(graph, *segment, station, derived.length_m);
      if (!evaluated.ok) {
