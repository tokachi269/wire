...
namespace city::wire::generation::detail {
...
inline bool normalize_xy(Vec3d* v) { return city::wire::NormalizeXY(v); }
...
inline Vec3d local_to_world_on_pole_local(const Transformd& tf, double yaw_deg, const Vec3d& local) {
  return city::wire::LocalPointToWorld(city::wire::BuildPoleFrame(tf, yaw_deg), local);
}
