...
bool C395_backbone_is_new_does_not_affect_pairs() {
  ...
  return !contains_text(body, "n.is_new") &&
         !contains_text(body, "made.nodes[n.id].is_new");
}

bool C396_backbone_existing_pole_does_not_read_existing_spans() {
  struct snapshot {
    city::wire::SpanLayoutRule rule{};
    city::wire::SpanLayoutEntry layout{};
    city::wire::CurveCacheEntry curve{};
    city::wire::BoundsCacheEntry bounds{};
    city::wire::SpanVisualCacheEntry visual{};
    city::wire::SpanRenderCacheEntry render{};
    std::uint64_t runtime_version = 0;
  };
  const auto same_points = [](const std::vector<city::wire::Vec3d>& lhs,
                              const std::vector<city::wire::Vec3d>& rhs) {
  ...
  const auto same_rule = [](const city::wire::SpanLayoutRule& lhs, const city::wire::SpanLayoutRule& rhs) {
    return lhs.span_id == rhs.span_id && lhs.flow_kind == rhs.flow_kind && lhs.pass_mode == rhs.pass_mode &&
           ...
           lhs.end.endpoint_node_id == rhs.end.endpoint_node_id &&
           lhs.end.port_id == rhs.end.port_id;
  };
  const auto same_layout = [](const city::wire::SpanLayoutEntry& lhs, const city::wire::SpanLayoutEntry& rhs) {
    return lhs.span_id == rhs.span_id && lhs.flow_kind == rhs.flow_kind && lhs.pass_mode == rhs.pass_mode &&
           lhs.variation_flow_key == rhs.variation_flow_key && lhs.lowering_kind == rhs.lowering_kind &&
  ...
  for (std::size_t i = 0; i < first_out.value.generated_span_ids.size(); ++i) {
    const city::wire::ObjectId span_id = first_out.value.generated_span_ids[i];
    const auto rules = state.span_layout_rules(span_id);
    const auto layout = state.span_layout(span_id);
    const auto* curve = state.find_curve_cache(span_id);
    const auto* bounds = state.find_bounds_cache(span_id);
...
bool C402_backbone_bundle_spec_does_not_affect_pairs() {
  ...
  city::wire::BackboneSpec a = poly3_req(single);
  const std::vector<city::wire::Vec3d> pa = pole_positions_for(single, a);

  city::wire::CoreState multi;
  city::wire::BackboneSpec b = poly3_req(multi);
  add_backbone_bundle(b, city::wire::BundleKind::kCommunication);
  const std::vector<city::wire::Vec3d> pb = pole_positions_for(multi, b);
...
bool C523_backbone_scope_gate_matches_entrypoint() {
  const std::filesystem::path entry = repo_root() / "domains" / "wire" / "src" / "generation" / "generate_from_backbone.cpp";
  const std::filesystem::path backbone = repo_root() / "domains" / "wire" / "src" / "generation" / "backbone" / "pipeline.cpp";
...
bool C601_backbone_context_only_bundle_policy_does_not_filter_new_route() {
  ...
  if (branch_bundle == nullptr || branch_bundle->bundle_template_id != city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kCommunication)) {
    return false;
  }

  const city::wire::ObjectId e = branch_out.value.generated_pole_ids.front();
  const auto* pole = state.view().poles().find(e);
  if (pole == nullptr) {
    return false;
  }
  city::wire::BackboneSpec cont = line_req(state);
  cont.bundles.clear();
  add_backbone_bundle(cont, city::wire::BundleKind::kLowVoltage);
  add_backbone_bundle(cont, city::wire::BundleKind::kCommunication);
...
bool C602_backbone_context_only_pole_band_does_not_filter_new_route() {
  ...
  const city::wire::ObjectId a = base.value.generated_pole_ids[0];
  const city::wire::ObjectId b = base.value.generated_pole_ids[1];
  city::wire::PoleTypeId comm_type_id = city::wire::kInvalidPoleTypeId;
  ...
  if (comm_type_id == city::wire::kInvalidPoleTypeId) {
    return false;
  }
  ...
  city::wire::PoleTypeDefinition comm_type = type_it->second;
...
bool C608_backbone_saved_backbone_result_does_not_duplicate_saved_pole_nodes() {
  ...
  std::unordered_set<city::wire::ObjectId> node_ids{};
  for (const city::wire::SupportNode& node : result.nodes) {
    if (node.node_id == city::wire::kInvalidObjectId || !node_ids.insert(node.node_id).second) {
      return false;
    }
...
bool C567_backbone_segment_pick_midair_uses_source_span_height() {
  ...
  for (city::wire::ObjectId span_id : out.value.generated_span_ids) {
    ...
    if ((a->owner_pole_id == city::wire::kInvalidObjectId && almost_equal(a->world_position.z, expected_z, 1e-9)) ||
        (b->owner_pole_id == city::wire::kInvalidObjectId && almost_equal(b->world_position.z, expected_z, 1e-9))) {
      return true;
    }
  }
  return false;
...
bool C568_backbone_source_edge_midair_branch_uses_source_curve_projection() {
  ...
  const std::optional<city::wire::Vec3d> expected = state.view().source_edge_projection_world(
      source_edge.edge_id, source_edge.node_a, city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kLowVoltage), 0, 0.5);
  ...
  const auto saved_midair = std::find_if(graph.nodes.begin(), graph.nodes.end(),
                                         [](const city::wire::SavedBackboneNode& saved) {
                                           return saved.pole_id == city::wire::kInvalidObjectId &&
  ...
  for (city::wire::ObjectId span_id : out.value.generated_span_ids) {
    ...
    const bool start_ownerless = a->owner_pole_id == city::wire::kInvalidObjectId;
    const bool end_ownerless = b->owner_pole_id == city::wire::kInvalidObjectId;
    const city::wire::LayoutEndpoint& endpoint = start_ownerless ? layout.entry->start : layout.entry->end;
    const city::wire::Port* ownerless_port = start_ownerless ? a : b;
...
bool C718_viewer_hit_world_height_is_not_source_edge_branch_authority() {
  ...
  const city::wire::SavedBackboneGraph& graph = state.view().backbone();
  const bool saved_source = std::any_of(graph.nodes.begin(), graph.nodes.end(), [](const city::wire::SavedBackboneNode& saved) {
    return saved.pole_id == city::wire::kInvalidObjectId && saved.has_source_edge &&
...
bool C719_source_edge_branch_endpoint_follows_current_curve_projection() {
  city::wire::CoreState state;
  city::wire::GeometrySettings initial_geometry = state.view().geometry_settings();
  ...
  const city::wire::ObjectId source_span_id = base_out.value.generated_span_ids.front();
  const city::wire::SavedBackboneEdge& source_edge = state.view().backbone().edges.front();
  const city::wire::ObjectId source_edge_id = source_edge.edge_id;
  const city::wire::ObjectId source_node_a = source_edge.node_a;
  const city::wire::ObjectId source_node_b = source_edge.node_b;
  const auto initial_projection = state.view().source_edge_projection_world(
      source_edge_id, source_node_a, city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kLowVoltage), 0, 0.5);
  ...
  const city::wire::ObjectId branch_span_id = branch_out.value.generated_span_ids.front();
  ...
  const city::wire::Span* branch_span = state.view().spans().find(branch_span_id);
  if (source_curve == nullptr || !current_projection.has_value() || !branch_layout.has_layout() ||
      branch_span == nullptr) {
    return false;
  }
  const city::wire::Port* start_port = state.view().ports().find(branch_layout.entry->start.port_id);
  const bool start_ownerless = start_port != nullptr && start_port->owner_pole_id == city::wire::kInvalidObjectId;
  const city::wire::LayoutEndpoint& endpoint = start_ownerless ? branch_layout.entry->start : branch_layout.entry->end;
...
bool C721_source_edge_identity_survives_projection_update() {
  ...
  const city::wire::ObjectId branch_span_id = branch_out.value.generated_span_ids.front();
  const city::wire::SpanLayoutView before_layout = state.span_layout(branch_span_id);
  if (!before_layout.has_layout()) {
    return false;
  }
  const city::wire::LayoutEndpoint& before_endpoint =
      before_layout.entry->start.source_projection.valid() ? before_layout.entry->start : before_layout.entry->end;
  const city::wire::SourceEdgeProjectionRef before_ref = before_endpoint.source_projection;
  if (!before_ref.valid()) {
    return false;
  }
  ...
  const city::wire::ObjectId source_node_id = saved_node_before->node_id;
  ...
  const city::wire::SpanLayoutView after_layout = state.span_layout(branch_span_id);
  if (!after_layout.has_layout()) {
    return false;
  }
  const city::wire::LayoutEndpoint& after_endpoint =
      after_layout.entry->start.source_projection.valid() ? after_layout.entry->start : after_layout.entry->end;
  const city::wire::SourceEdgeProjectionRef after_ref = after_endpoint.source_projection;
...
bool C722_unresolved_source_edge_reference_fails_before_mutation() {
  ...
  city::wire::SupportNode pending{};
  pending.node_id = state.next_id();
  pending.support_kind = city::wire::SupportKind::kMidair;
  ...
  pending.source_edge_node_b_id = city::wire::kInvalidObjectId;
  pending.source_edge_t = 0.5;
  city::wire::CoreStateTestHook::pending_support_nodes(state).push_back(pending);
...
bool C723_source_edge_branch_does_not_change_source_sag() {
  ...
  const city::wire::ObjectId source_span_id = base_out.value.generated_span_ids.front();
  const city::wire::CurveCacheEntry* before_curve = state.find_curve_cache(source_span_id);
  if (before_curve == nullptr || before_curve->detail.sample_points.empty()) {
    return false;
  }
  const std::vector<city::wire::Vec3d> before_samples = before_curve->detail.sample_points;
  ...
  branch.path.node_specs = {node};
  const auto branch_out = state.GenerateFromBackboneSpec(branch);
  const city::wire::CurveCacheEntry* after_curve = state.find_curve_cache(source_span_id);
...
bool C724_source_template_sag_change_updates_branch_projection() {
  ...
  const city::wire::Bundle* source_bundle = source_span == nullptr ? nullptr : state.view().bundles().find(source_span->bundle_id);
  ...
  city::wire::CableTemplate edited_cable_template = cable_template_it->second;
  const city::wire::SavedBackboneEdge source_edge = state.view().backbone().edges.front();
...
bool C725_source_layout_settings_update_keeps_branch_projection_current() {
  ...
  const auto before_projection = state.view().source_edge_projection_world(
      source_edge.edge_id, source_edge.node_a, city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kLowVoltage), 0, kSourceT);
  if (!before_projection.has_value()) {
    return false;
  }
...
bool C726_source_edge_branch_projection_does_not_require_prior_curve_cache() {
  ...
  const city::wire::ObjectId source_span_id = base_out.value.generated_span_ids.front();
  const city::wire::SpanLayoutView source_layout = state.span_layout(source_span_id);
  ...
  const city::wire::SpanLayoutView branch_layout = state.span_layout(branch_out.value.generated_span_ids.front());
...
bool C720_source_edge_pipeline_front_half_does_not_read_curve_projection() {
  std::string text;
  if (!file_text(repo_root() / "domains" / "wire" / "src" / "generation" / "backbone" / "pipeline.cpp", &text)) {
    return false;
  }
...
bool C612_backbone_direct_derive_does_not_call_recalc_paths() {
  std::string text;
  if (!file_text(repo_root() / "domains/wire/src/generation/backbone/derive.cpp", &text)) {
    return false;
  }
...
bool C617_backbone_reshape_does_not_rewrite_layout() {
  ...
  const city::wire::SpanLayoutView before = state.span_layout(span_id);
  if (!before.has_layout()) {
    return false;
  }
  const city::wire::SpanLayoutEntry before_layout = *before.entry;
  ...
  const city::wire::SpanLayoutView after = state.span_layout(span_id);
...
bool C618_backbone_redraw_does_not_rewrite_layout_or_geom() {
  ...
  const city::wire::BoundsCacheEntry* before_bounds = state.find_bounds_cache(span_id);
  ...
  const std::vector<city::wire::Vec3d> before_samples = before_curve->detail.sample_points;
  const city::wire::AABBd before_whole = before_bounds->whole;
  ...
  const city::wire::BoundsCacheEntry* after_bounds = state.find_bounds_cache(span_id);
...
bool C620_backbone_update_boundary_has_no_operation_specific_kinds() {
  std::string h;
  std::string cpp;
  if (!file_text(repo_root() / "domains/wire/include/city/wire/core_runtime_types.hpp", &h) ||
      !file_text(repo_root() / "domains/wire/src/generation/backbone/derive.cpp", &cpp)) {
    return false;
  }
...
bool C784_backbone_fixture_plan_is_operation_scoped_for_reposition_and_materialization() {
  ...
  if (!file_text(repo_root() / "domains/wire/src/generation/backbone/derive.cpp", &derive_source) ||
      !file_text(repo_root() / "domains/wire/src/generation/backbone/model_assembly.cpp", &model_source) ||
...
bool C826_geometry_generation_tolerances_are_named() {
  WIRE_TEST_EXPECT(std::filesystem::exists(repo_root() / "domains/wire/include/city/wire/support/numeric_tolerances.hpp"),
                   "support/numeric_tolerances.hpp is missing");
  WIRE_TEST_EXPECT(!std::filesystem::exists(repo_root() / "domains/wire/include/city/wire/numeric_tolerances.hpp"),
                   "old numeric_tolerances.hpp path remains");
  const std::vector<std::filesystem::path> paths = {
      repo_root() / "domains/wire/include/city/wire/coord_utils.hpp",
      repo_root() / "domains/wire/src",
  };
  const std::vector<std::string> banned = {
      "1e-6",
      "1e-9",
...
bool C831_normalization_fallback_calls_are_classified_in_source() {
  const std::vector<std::pair<std::filesystem::path, std::string>> required = {
      {repo_root() / "domains/wire/src/geometry/curve/curve.cpp", "R5 fallback: legitimate degenerate cable frame"},
      {repo_root() / "domains/wire/src/generation/backbone/curve_parts.cpp", "R5 fallback: legitimate degenerate patch tangent"},
      {repo_root() / "domains/wire/src/generation/backbone/span_visual_assembly.cpp",
       "R5 fallback: legitimate degenerate visual sampling"},
      {repo_root() / "domains/wire/src/generation/backbone/emit_shared.cpp",
...
bool C788_model_assembly_keeps_belt_and_socket_authority_in_materialization() {
  ...
  if (!file_text(repo_root() / "domains/wire/src/generation/backbone/model_assembly.cpp",
                 &model_assembly) ||
      !file_text(repo_root() / "domains/wire/src/generation/backbone/curve_parts.cpp",
                 &curve_parts) ||
...
bool C793_backbone_derive_uses_canonical_rule_order_and_lookup() {
  ...
  if (!file_text(repo_root() / "domains/wire/src/generation/backbone/derive.cpp", &cpp) ||
