...
bool test_bundle_query_spans_by_bundle() {
  ...
  const auto add_bundle =
      state.AddBundle(2, 0.2, city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kLowVoltage));
  if (!add_bundle.ok) {
    return false;
  }
...
bool test_center_band_ports_are_offset_from_pole_centerline() {
  ...
  for (const auto* port : detail.owned_ports) {
    if (port == nullptr || port->template_side != city::wire::SlotSide::kCenter) {
      continue;
    }
    checked_center = true;
    const city::wire::Vec3d local = to_local_on_pole_test(*detail.pole, port->world_position);
...
bool test_template_type_ownership_is_separated() {
  constexpr bool bundle_has_flag = has_allow_midair_branch_member<city::wire::BundleTemplate>::value;
  constexpr bool cable_has_flag = has_allow_midair_branch_member<city::wire::CableTemplate>::value;
  constexpr bool span_has_bezier = has_bezier_control_points_member<city::wire::Span>::value;
  constexpr bool bundle_has_bezier = has_bezier_control_points_member<city::wire::Bundle>::value;
  constexpr bool span_has_arc_length = has_arc_length_table_member<city::wire::Span>::value;
  constexpr bool bundle_has_arc_length = has_arc_length_table_member<city::wire::Bundle>::value;
...
bool test_cable_template_edit_preserves_pole_tilt_instance_value() {
  ...
  city::wire::CableTemplate cable = cables.begin()->second;
...
bool test_pole_tilt_biases_toward_incident_span_direction() {
  ...
  city::wire::Transformd a_tf{};
  a_tf.position = {0.0, 0.0, 0.0};
  city::wire::Transformd b_tf{};
  ...
  const auto add = add_connection_by_category(state, pole_a, pole_b, city::wire::ConnectionCategory::kLowVoltage);
  if (!add.ok) {
    return false;
  }
  ...
  city::wire::Vec3d top_world = city::wire::LocalPointToWorld(
      city::wire::BuildPoleFrame(pole->world_transform, pole_view->layout_yaw_deg), {0.0, 0.0, pole->height_m});
  city::wire::Vec3d tilt_dir = top_world - pole->world_transform.position;
  tilt_dir.z = 0.0;
  city::wire::Vec3d span_dir = remote_port->world_position - pole->world_transform.position;
  span_dir.z = 0.0;
  if (!city::wire::NormalizeXY(&tilt_dir) || !city::wire::NormalizeXY(&span_dir)) {
    return false;
  }
  return city::wire::Dot(tilt_dir, span_dir) > 0.5;
}

bool test_pole_tilt_magnitude_increases_with_pull_imbalance() {
  auto measure_center_tilt = [](const city::wire::Vec3d& left, const city::wire::Vec3d& right) -> double {
    CoreState state;
    const auto type_ids = sorted_pole_type_ids(state);
    if (type_ids.empty()) {
      return -1.0;
    ...
    if (!add_connection_by_category(state, pole_b, pole_a, city::wire::ConnectionCategory::kLowVoltage).ok ||
        !add_connection_by_category(state, pole_b, pole_c, city::wire::ConnectionCategory::kLowVoltage).ok) {
      return -1.0;
    }
