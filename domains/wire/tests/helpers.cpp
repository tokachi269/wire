...
double length_xy(const city::wire::Vec3d& v) { return std::sqrt(v.x * v.x + v.y * v.y); }

double distance3d(const city::wire::Vec3d& a, const city::wire::Vec3d& b) {
  const city::wire::Vec3d d = a - b;
  return std::sqrt(d.x * d.x + d.y * d.y + d.z * d.z);
}

double distance_xy(const city::wire::Vec3d& a, const city::wire::Vec3d& b) {
  const city::wire::Vec3d d = a - b;
  return std::sqrt(d.x * d.x + d.y * d.y);
}

double axis_angle_deg(const city::wire::Vec3d& a, const city::wire::Vec3d& b) {
  const city::wire::Vec3d na = normalize_xy_safe(a);
  const city::wire::Vec3d nb = normalize_xy_safe(b);
...
city::wire::Vec3d farthest_pair_axis_xy(const std::vector<city::wire::Vec3d>& points) {
  city::wire::Vec3d best{0.0, 0.0, 0.0};
  double best_len2 = 0.0;
  for (std::size_t i = 0; i < points.size(); ++i) {
    for (std::size_t j = i + 1; j < points.size(); ++j) {
      const city::wire::Vec3d d = points[j] - points[i];
...
double min_pairwise_distance3d(const std::vector<city::wire::Vec3d>& points) {
  if (points.size() < 2) {
    return 0.0;
  }
...
double min_pairwise_distance_xy(const std::vector<city::wire::Vec3d>& points) {
  if (points.size() < 2) {
    return 0.0;
  }
  double min_distance = std::numeric_limits<double>::infinity();
...
double mean_pairwise_distance_xy(const std::vector<city::wire::Vec3d>& points) {
  if (points.size() < 2) {
    return 0.0;
  }
  double sum = 0.0;
...
const city::wire::Span* find_span_by_ports(const CoreState& state, ObjectId port_a_id, ObjectId port_b_id) {
  for (const city::wire::Span& span : state.view().edit_state().spans.items()) {
...
struct ParsedCaptureRequest {
  ...
  struct BackboneEdge {
    ObjectId node_a_id = city::wire::kInvalidObjectId;
    ObjectId node_b_id = city::wire::kInvalidObjectId;
  };
  struct CurrentSpan {
    ObjectId span_id = city::wire::kInvalidObjectId;
    ObjectId bundle_id = city::wire::kInvalidObjectId;
    ObjectId endpoint_node_a_id = city::wire::kInvalidObjectId;
    ObjectId endpoint_node_b_id = city::wire::kInvalidObjectId;
  };
  ...
  struct CurrentPole {
    ObjectId pole_id = city::wire::kInvalidObjectId;
    city::wire::PoleTypeId pole_type_id = city::wire::kInvalidPoleTypeId;
  };
...
bool parse_vec3_test(const std::string& text, city::wire::Vec3d* out) {
  if (out == nullptr) {
    return false;
  }
  char comma_a = 0;
...
city::wire::SupportKind parse_support_kind_test(const std::string& text) {
  if (text == "Midair") {
    return city::wire::SupportKind::kMidair;
  }
  if (text == "Building") {
    return city::wire::SupportKind::kExternal;
  }
  if (text == "Ground") {
    return city::wire::SupportKind::kGround;
  }
  return city::wire::SupportKind::kPole;
}

city::wire::PathDirectionMode parse_direction_mode_test(const std::string& text) {
  if (text == "Forward") {
    return city::wire::PathDirectionMode::kForward;
  }
  if (text == "Reverse") {
    return city::wire::PathDirectionMode::kReverse;
  }
  return city::wire::PathDirectionMode::kAuto;
}
...
std::optional<ParsedCaptureRequest> parse_capture_request_test(const std::filesystem::path& path, std::string* error) {
  ...
  std::map<std::size_t, city::wire::Vec3d> path_points{};
  std::map<std::size_t, city::wire::BackboneInputSpec::NodeSpec> node_specs{};
  std::map<std::size_t, city::wire::BackboneBundleSpec> bundles{};
  ...
  while (std::getline(ifs, line)) {
    ...
    if (key == "request.pole_type_id") {
      parsed.spec.pole_type_id = static_cast<city::wire::PoleTypeId>(std::stoul(value));
      continue;
    }
    ...
    if (parse_indexed_key_test(key, "request.bundle[", &index, &suffix)) {
      ...
      if (suffix == ".kind") {
        bundle.bundle_template_id = city::wire::DefaultBundleTemplateId(static_cast<city::wire::BundleKind>(std::stoi(value)));
      } else if (suffix == ".layer") {
        bundle.layer = static_cast<city::wire::SpanLayer>(std::stoi(value));
    ...
    if (parse_indexed_key_test(key, "result.current_bundle[", &index, &suffix)) {
      ...
      if (suffix == ".bundle_id") {
        ...
        bundle.bundle_template_id = static_cast<city::wire::BundleTemplateId>(std::stoul(value));
      }
      continue;
    }
    if (parse_indexed_key_test(key, "result.current_pole[", &index, &suffix)) {
      ...
      if (suffix == ".pole_id") {
        ...
        pole.pole_type_id = static_cast<city::wire::PoleTypeId>(std::stoul(value));
      }
      continue;
    }
  }
...
make_backbone_fixture(CoreState& state, const std::vector<city::wire::Vec3d>& points,
                 const std::vector<city::wire::BundleKind>& bundles) {
  city::wire::EditResult<BackboneFixture> out{};
  ...
  const auto first_template = state.view().bundle_templates().find(city::wire::DefaultBundleTemplateId(bundles.front()));
  ...
  if (request.pole_type_id == city::wire::kInvalidPoleTypeId) {
  ...
  for (city::wire::BundleKind kind : bundles) {
    city::wire::BackboneBundleSpec spec{};
    spec.bundle_template_id = city::wire::DefaultBundleTemplateId(kind);
...
bool almost_equal(const city::wire::Vec3d& a, const city::wire::Vec3d& b, double eps) {
...
double dot_xy(const city::wire::Vec3d& a, const city::wire::Vec3d& b) { return a.x * b.x + a.y * b.y; }

city::wire::Vec3d local_side_axis_from_yaw(double yaw_deg) {
  constexpr double kPi = 3.14159265358979323846;
  const double rad = yaw_deg * (kPi / 180.0);
  return normalize_xy_safe(city::wire::Vec3d{-std::sin(rad), std::cos(rad), 0.0});
}
...
double effective_pole_yaw_deg_test(const city::wire::Pole& pole) { return pole.world_transform.rotation_euler_deg.z; }

city::wire::Vec3d to_local_on_pole_test(const city::wire::Pole& pole, const city::wire::Vec3d& world) {
  const city::wire::Vec3d delta = world - pole.world_transform.position;
...
bool aabb_valid(const city::wire::AABBd& aabb) {
  return aabb.min.x <= aabb.max.x && aabb.min.y <= aabb.max.y &&
         city::wire::HeightAlongWorldUp(aabb.min) <= city::wire::HeightAlongWorldUp(aabb.max);
}
...
ObjectId find_pole_id_by_position(const CoreState& state, const city::wire::Vec3d& pos, double eps) {
...
void add_backbone_bundle(city::wire::BackboneSpec& req, city::wire::BundleKind template_id,
                         city::wire::SpanLayer layer, int count) {
  city::wire::BackboneBundleSpec bundle{};
  bundle.bundle_template_id = city::wire::DefaultBundleTemplateId(template_id);
  bundle.layer = layer;
  bundle.count = count;
...
city::wire::BundleTemplateId bundle_template_for_category_test(city::wire::ConnectionCategory category) {
  ...
  case city::wire::ConnectionCategory::kHighVoltage:
    return city::wire::kDefaultHighVoltageBundleTemplateId;
  case city::wire::ConnectionCategory::kCommunication:
    return city::wire::kDefaultCommunicationBundleTemplateId;
  case city::wire::ConnectionCategory::kOptical:
    return city::wire::kDefaultOpticalBundleTemplateId;
  case city::wire::ConnectionCategory::kDrop:
    return city::wire::kDefaultDropBundleTemplateId;
  case city::wire::ConnectionCategory::kLowVoltage:
  default:
    return city::wire::kDefaultLowVoltageBundleTemplateId;
  }
}

city::wire::ConnectionCategory category_for_bundle_template_test(city::wire::BundleTemplateId bundle_template_id) {
  switch (bundle_template_id) {
  case city::wire::kDefaultHighVoltageBundleTemplateId:
    return city::wire::ConnectionCategory::kHighVoltage;
  case city::wire::kDefaultCommunicationBundleTemplateId:
    return city::wire::ConnectionCategory::kCommunication;
  case city::wire::kDefaultOpticalBundleTemplateId:
    return city::wire::ConnectionCategory::kOptical;
  case city::wire::kDefaultDropBundleTemplateId:
    return city::wire::ConnectionCategory::kDrop;
  case city::wire::kDefaultLowVoltageBundleTemplateId:
  default:
    return city::wire::ConnectionCategory::kLowVoltage;
  }
}

AxisRelationMetrics measure_pole_axis_relation_metrics(const CoreState& state, ObjectId pole_id, city::wire::PortLayer layer,
  ...
  std::vector<city::wire::Vec3d> port_points{};
  for (const city::wire::Port& port : state.view().edit_state().ports.items()) {
    if (port.owner_pole_id == pole_id && port.layer == layer &&
        port.placement_source != city::wire::PortPlacementSourceKind::kBranchSupport) {
...
BranchRunoutMetrics measure_branch_runout_metrics(const CoreState& state, ObjectId span_id) {
  ...
  const bool branch_at_start = layout->start.origin == city::wire::LayoutOriginKind::kBranchSupport ||
                               layout->start.flow_kind == city::wire::BackboneFlowKind::kBranch ||
                               layout->start.local_departure_length_m >= layout->end.local_departure_length_m;
  const city::wire::Vec3d root = branch_at_start ? curve->detail.EvaluatePosition(0.0) : curve->detail.EvaluatePosition(1.0);
  const city::wire::Vec3d other =
      branch_at_start ? curve->detail.EvaluatePosition(1.0) : curve->detail.EvaluatePosition(0.0);
  const city::wire::Vec3d chord_axis = normalize_xy_safe(other - root);
  if (length_xy(chord_axis) <= 1e-9) {
    return metrics;
  }
  const city::wire::Vec3d lateral_axis{-chord_axis.y, chord_axis.x, 0.0};
...
add_connection_by_category(city::wire::CoreState& state, city::wire::ObjectId pole_a_id, city::wire::ObjectId pole_b_id,
  ...
  city::wire::EditResult<FixtureConnectionResult> out{};
  const city::wire::Pole* a = state.view().poles().find(pole_a_id);
  const city::wire::Pole* b = state.view().poles().find(pole_b_id);
  if (a == nullptr || b == nullptr || pole_a_id == pole_b_id) {
    out.error = "fixture poles are invalid";
    return out;
  }
  const city::wire::BundleTemplateId kind =
  ...
  const int count = tmpl->second.count_rule == city::wire::BundleCountRuleKind::kFixed
  ...
  const city::wire::PortLayer port_layer =
      tmpl->second.default_layer == city::wire::SpanLayer::kHighVoltage
          ? city::wire::PortLayer::kHighVoltage
          : tmpl->second.default_layer == city::wire::SpanLayer::kCommunication
                ? city::wire::PortLayer::kCommunication
                : city::wire::PortLayer::kLowVoltage;
  const city::wire::Vec3d pa = a->world_transform.position + city::wire::Vec3d{0.0, 0.0, 9.0};
  const city::wire::Vec3d pb = b->world_transform.position + city::wire::Vec3d{0.0, 0.0, 9.0};
  const auto port_a = state.AddPort(pole_a_id, pa, city::wire::PortKind::kPower, port_layer);
  const auto port_b = state.AddPort(pole_b_id, pb, city::wire::PortKind::kPower, port_layer);
  if (!port_a.ok || !port_b.ok) {
    out.error = "fixture port creation failed";
    return out;
  }
  const auto span = state.AddSpan(port_a.value, port_b.value, city::wire::SpanKind::kDistribution,
...
bool has_selected_port_in_candidates(const city::wire::PortResolutionDebugRecord& record) {
  if (record.selected_port_id == city::wire::kInvalidObjectId) {
    return true;
  }
...
bool restore_capture_request_scene(const std::filesystem::path& capture_path, CoreState& state,
                                   city::wire::BackboneSpec* remapped_spec, std::string* error) {
  ...
  *remapped_spec = parsed->spec;
  ...
  std::unordered_map<ObjectId, city::wire::BundleTemplateId> bundle_templates_by_id{};
  for (const auto& bundle : parsed->current_bundles) {
    if (bundle.bundle_id != city::wire::kInvalidObjectId) {
      bundle_templates_by_id[bundle.bundle_id] = bundle.bundle_template_id;
    }
  }
  std::unordered_map<std::uint64_t, std::vector<city::wire::BundleTemplateId>> restore_templates_by_edge{};
  city::wire::PoleTypeId restore_pole_type_id = city::wire::kInvalidPoleTypeId;
  auto supports_bundle_template = [&](city::wire::PoleTypeId pole_type_id, city::wire::BundleTemplateId template_id) {
  ...
  auto supports_restore_templates = [&](city::wire::PoleTypeId pole_type_id) {
    if (bundle_templates_by_id.empty()) {
      return supports_bundle_template(pole_type_id, city::wire::kDefaultLowVoltageBundleTemplateId);
    }
  ...
  std::unordered_map<city::wire::PoleTypeId, int> current_pole_type_counts{};
  for (const auto& pole : parsed->current_poles) {
    if (pole.pole_id == city::wire::kInvalidObjectId || excluded_generated_poles.contains(pole.pole_id) ||
        pole.pole_type_id == city::wire::kInvalidPoleTypeId) {
      continue;
    }
  ...
  if (restore_pole_type_id == city::wire::kInvalidPoleTypeId &&
  ...
  for (const auto& span : parsed->current_spans) {
    ...
    if (span.bundle_id == city::wire::kInvalidObjectId || span.endpoint_node_a_id == city::wire::kInvalidObjectId ||
        span.endpoint_node_b_id == city::wire::kInvalidObjectId || span.endpoint_node_a_id == span.endpoint_node_b_id) {
      continue;
    }
  ...
  for (ObjectId span_id : parsed->last_generated_span_ids) {
    ...
    if (span.endpoint_node_a_id == city::wire::kInvalidObjectId ||
        span.endpoint_node_b_id == city::wire::kInvalidObjectId ||
  ...
  std::unordered_map<ObjectId, city::wire::Vec3d> backbone_positions_by_node_id{};
  ...
  for (const auto& node : parsed->backbone_nodes) {
    if (node.node_id == city::wire::kInvalidObjectId || excluded_generated_poles.contains(node.node_id)) {
      continue;
    }
    ...
    const auto add_pole = state.AddPole(tf, 10.0, "ReplayPole", city::wire::PoleKind::kConcrete,
                                        city::wire::PlacementMode::kManual);
    if (!add_pole.ok) {
      if (error != nullptr) {
  ...
  for (const auto& edge : parsed->backbone_edges) {
    if (edge.node_a_id == city::wire::kInvalidObjectId || edge.node_b_id == city::wire::kInvalidObjectId ||
    ...
    std::vector<city::wire::BundleTemplateId> restore_templates =
        (templates_it == restore_templates_by_edge.end() || templates_it->second.empty())
            ? std::vector<city::wire::BundleTemplateId>{city::wire::kDefaultLowVoltageBundleTemplateId}
            : templates_it->second;
    ...
    city::wire::BackboneSpec edge_request{};
    ...
    city::wire::BackboneInputSpec::NodeSpec start{};
    start.point_index = 0;
    start.support_kind = city::wire::SupportKind::kPole;
    start.node_id = it_a->second;
    city::wire::BackboneInputSpec::NodeSpec end{};
    end.point_index = 1;
    end.support_kind = city::wire::SupportKind::kPole;
    end.node_id = it_b->second;
    edge_request.path.node_specs.push_back(start);
    edge_request.path.node_specs.push_back(end);
    for (city::wire::BundleTemplateId bundle_template_id : restore_templates) {
      city::wire::BackboneBundleSpec bundle{};
  ...
  for (auto& node_spec : remapped_spec->path.node_specs) {
    if (node_spec.node_id == city::wire::kInvalidObjectId) {
      continue;
    }
    const auto it = remapped_node_ids.find(node_spec.node_id);
    if (it == remapped_node_ids.end()) {
      node_spec.node_id = city::wire::kInvalidObjectId;
      node_spec.support_kind = city::wire::SupportKind::kPole;
    } else {
