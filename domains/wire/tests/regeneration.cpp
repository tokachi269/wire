...
bool test_port_position_mode_defaults_auto() {
  ...
  return port->position_mode == city::wire::PortPositionMode::kAuto && !port->user_edited_position;
}
...
bool test_port_manual_set_and_reset_to_auto() {
  ...
  const city::wire::Vec3d manual_pos{123.0, 45.0, 9.0};
  ...
  if (manual_port->position_mode != city::wire::PortPositionMode::kManual ||
      manual_port->placement_source != city::wire::PortPlacementSourceKind::kManualEdit ||
  ...
  return reset_port->position_mode == city::wire::PortPositionMode::kAuto && !reset_port->user_edited_position &&
         reset_port->placement_source != city::wire::PortPlacementSourceKind::kManualEdit &&
...
bool test_manual_port_not_overwritten_by_auto_relayout() {
  ...
  const city::wire::Vec3d manual_pos{77.0, -12.0, 8.0};
  ...
  return port_after->position_mode == city::wire::PortPositionMode::kManual &&
...
bool test_move_pole_reprojects_auto_ports_and_preserves_manual_ports() {
  ...
  ObjectId auto_port_id = city::wire::kInvalidObjectId;
  ObjectId manual_port_id = city::wire::kInvalidObjectId;
  city::wire::Vec3d auto_before{};
  city::wire::Vec3d manual_before{};
  for (const auto& port : state.view().edit_state().ports.items()) {
    if (port.owner_pole_id != pole_id) {
      continue;
    }
    if (port.template_side == city::wire::SlotSide::kLeft) {
      auto_port_id = port.id;
      auto_before = port.world_position;
    } else if (port.template_side == city::wire::SlotSide::kRight) {
      manual_port_id = port.id;
      manual_before = port.world_position;
    }
  }
  if (auto_port_id == city::wire::kInvalidObjectId || manual_port_id == city::wire::kInvalidObjectId) {
    return false;
  }
  ...
  return auto_moved && manual_kept && manual_after->position_mode == city::wire::PortPositionMode::kManual;
}
...
bool test_generate_from_guide_keeps_manual_boundaries_stable() {
  ...
  auto find_pole_at = [&](const city::wire::Vec3d& pos) -> const city::wire::Pole* {
    const city::wire::Pole* found = nullptr;
    for (const auto& pole : state.view().edit_state().poles.items()) {
      if (almost_equal(pole.world_transform.position, pos)) {
        if (found == nullptr || pole.placement_mode == city::wire::PlacementMode::kManual) {
          found = &pole;
        }
      }
    }
  ...
  if (!state.SetPolePlacementMode(start_id, city::wire::PlacementMode::kManual).ok ||
      !state.SetPolePlacementMode(mid_id, city::wire::PlacementMode::kManual).ok) {
    return false;
  }
  start_before = state.view().edit_state().poles.find(start_id);
  mid_before = state.view().edit_state().poles.find(mid_id);
  if (start_before == nullptr || mid_before == nullptr ||
      start_before->placement_mode != city::wire::PlacementMode::kManual ||
      mid_before->placement_mode != city::wire::PlacementMode::kManual) {
    return false;
  }

  req.path.polyline = {{20.0, 0.0, 0.0}, {40.0, 0.0, 0.0}};
  city::wire::BackboneInputSpec::NodeSpec mid_spec{};
  mid_spec.point_index = 0;
  mid_spec.support_kind = city::wire::SupportKind::kPole;
  ...
  return almost_equal(start_after->world_transform.position, start_pos_before) &&
         ...
         start_after->placement_mode == city::wire::PlacementMode::kManual &&
         mid_after->placement_mode == city::wire::PlacementMode::kManual;
}
...
bool test_generate_from_guide_vertices_are_not_forced_manual_by_default() {
  ...
  return start->placement_mode == city::wire::PlacementMode::kAuto &&
         end->placement_mode == city::wire::PlacementMode::kAuto &&
         mid->placement_mode == city::wire::PlacementMode::kAuto;
}
...
bool test_generate_from_guide_pin_vertices_option() {
  ...
  for (const auto& pole : state.view().edit_state().poles.items()) {
    if (almost_equal(pole.world_transform.position, city::wire::Vec3d{10.0, 0.0, 0.0})) {
      return pole.placement_mode == city::wire::PlacementMode::kManual;
    }
  }
  return false;
}
...
bool test_set_pole_placement_mode_auto_manual_roundtrip() {
  ...
  if (pole == nullptr || pole->placement_mode != city::wire::PlacementMode::kAuto) {
    return false;
  }
  const auto to_manual = state.SetPolePlacementMode(pole_id, city::wire::PlacementMode::kManual);
  if (!to_manual.ok) {
    return false;
  }
  ...
  const auto to_auto = state.SetPolePlacementMode(pole_id, city::wire::PlacementMode::kAuto);
  if (!to_auto.ok) {
    return false;
  }
  pole = state.view().edit_state().poles.find(pole_id);
  return pole != nullptr && pole->placement_mode == city::wire::PlacementMode::kAuto && !pole->user_edited;
}
...
bool test_set_pole_flip180_updates_ports() {
  ...
  const city::wire::Vec3d before_pos = port_before->world_position;
