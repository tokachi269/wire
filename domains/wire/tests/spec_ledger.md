...
| C866 | Behavior | Invariant | range Bundleの個別count更新はexact Bundle identityとlane prefix identityを保ち、無関係Bundleを判断入力・変更対象にせず、save/load後もbindingを再構築できる | `presence` `anchor` `differential` | template共有Bundleを巻き込む、存続laneを置換する、退役lane relationを残す回帰を防ぐ |
| C867 | Behavior | FailureAtomicity | range Bundleの退役laneにuser Attachmentまたはmanual Portがあるcount減少はauthority不変で拒否する | `presence` `differential` | user-owned relationを暗黙削除する、または失敗後に一部だけretireする回帰を防ぐ |
| C868 | Behavior | Boundary | fixed BundleTemplateの個別Bundle count変更とrange外count要求はauthority不変で拒否する | `presence` `differential` | count policyを個別overrideとして迂回する回帰を防ぐ |
| C869 | Behavior | Invariant | 同一3点route上のexact Bundleだけを完全退役し、Bundle・EdgeBundle・Span・専用Port・binding・continuity・default Attachment・derived output・runtime indexを消す一方、peer Bundle identityとtopologyを変えずsave/load後も同じ状態を保つ | `presence` `anchor` `differential` | exact scope外を巻き込む、退役relationを残す、load後に削除済みtopologyを復活させる回帰を防ぐ |
| C870 | Behavior | Boundary | cross/sharpを含むexact Bundleの全saved componentをidentityで退役して対象derived outputだけを消す。source-edge branch Bundleはsourceを残して退役でき、source Bundleに存続projectionがある場合はauthority不変で拒否する | `presence` `oracle` `differential` | branch/cross/sharp/source-edgeを一律unsupportedにする、derived接続曲線を残す、source dependencyを切断する回帰を防ぐ |
| C871 | Behavior | FailureAtomicity | exact BundleのSpanにuser AttachmentまたはSpan override、もしくはbound Portにmanual editがある退役要求はauthority不変で拒否する | `presence` `differential` | user-owned authorityを暗黙削除する、または部分退役後に失敗する回帰を防ぐ |
| C872 | Behavior | Boundary | exact Bundle scopeの必須SpanBindingが不足する退役要求はfail-closedでauthorityを変更しない | `presence` `differential` | incomplete binding scopeをsilent skipしてBundleだけ削除する回帰を防ぐ |
| C873 | Behavior | Invariant | exact anchor Bundleから別placement identityを追加し、全physical edge membershipとlane continuityを同型に保つ一方、Bundle/Port/Span identityを共有せずpeerを変更しない。save/load後も維持し、追加Bundleの退役でanchor topologyへ戻る | `anchor` `presence` `differential` | template一致だけの再利用、Port/Span共有、部分component生成、stale binding/cacheを防ぐ |
| C874 | Behavior | Invariant | branch/cross/sharpのexact anchor relationをcategoryや通常pair候補から再判断せず新Bundleへ同型複製し、Jumper/NodePatchは既存derived経路から生成する | `anchor` `oracle` `presence` | branch除外、cross component混同、sharp continuityの再判定、parallel derived実装を防ぐ |
| C875 | Behavior | FailureAtomicity | placement_key重複とexact source Bundle mappingを持たないsource-edge dependent anchorはmutation前に拒否してauthorityを変更しない | `differential` `anchor` | identity衝突とsource dependency推測、部分mutationを防ぐ |
| C876 | Behavior | Boundary | anchorの必須SpanBindingまたはmulti-lane row continuityが部分欠落したBundle instance追加はfail-closedでauthorityを変更しない | `differential` `presence` | incomplete scopeやlane relationをsilent skipし、欠落したmembershipを複製成功扱いする回帰を防ぐ |
| C877 | Behavior | Invariant | exact currentとdesired concrete Bundle集合が同一ならsemantic stateを変えず、同keyのplacementとkRange count変更ではBundle ObjectIdと存続lane identityを維持する | `anchor` `differential` | no-opでID/counterを進める、survivorをretire/addへ置換する、count変更で存続laneを置換する回帰を防ぐ |
| C878 | Behavior | Invariant | current/desiredのkey差分をouter trial内でadd/retireし、A+B、Aへの縮小、A+BからA+Cへの置換、2 -> 4 -> 2を通じて存続Bundle identityとexact membershipを維持する | `anchor` `presence` `differential` | partial commit、scope外retire、survivor identity置換、stale relationを防ぐ |
| C879 | Behavior | Invariant | 3点以上およびbranch/cross/sharpのsaved relationを既存Add/Retireだけでreconcileし、desired/current入力順を反転しても同じkey別semantic topologyを得てsave/load後も維持する | `metamorphic` `anchor` `differential` | input vector順を新しいBundle ordering authorityにする、branch membershipやpairingを再判断する回帰を防ぐ |
| C880 | Behavior | FailureAtomicity | survive placement/countとaddの後にlate retireがuser authorityで失敗する場合、およびsource-edge addがunsupportedとなる場合もouter transaction全体を元authorityへ戻す | `differential` `presence` | nested generic operationの途中commitを本stateへ漏らす回帰を防ぐ |
| C881 | Behavior | Boundary | duplicate/nonzero key、template/layer/count/spacing、current-scope外anchor、anchor lane topology不一致をmutation前またはouter trial内で拒否し、scope外の同値keyは別correlationとして扱う | `differential` `anchor` | category/template/geometryによるscopeやanchor推測、placement keyのglobal identity化、silent normalization、暗黙template migrationを防ぐ |
| C882 | Behavior | Invariant | Generate/Apply transactionはその場のresolved resultとcommitするplacement identityを厳密対応させる一方、loadはstructurally validなdescriptorとsaved exact concrete scopeを復元しcurrent resolverを再実行しない | `anchor` `differential` `presence` | session-only recipe、transaction時の非resolver identity、resolver変更によるload不能、load時reroll、scope identity欠落を防ぐ |
| C883 | Behavior | Differential | explicit Applyは同じplacement keyのBundle identityを維持してdensity/height/lateralを更新し、失敗時はdescriptorとconcrete authorityを同時に元へ戻す | `anchor` `differential` `presence` | Apply途中のpartial commit、survivor置換、descriptorとconcrete topologyのずれを防ぐ |
| C884 | Behavior | Invariant | 1 -> 0 -> save/load -> Extend -> 1のexplicit operationはhidden Bundleを残さず、SavedBackboneGraphに保持したedge skeletonとvariationのrow continuityを延長済みscopeへ更新して同じplacement identityへ再materializeする | `anchor` `differential` `presence` | 0件時またはExtend時のgraph retention喪失、variation側node/edge geometry複製の復活、category/geometryからのroute推測、pairing再判断を防ぐ |
| C885 | Behavior | Invariant | exact variation extensionで保存したbranch/cross/sharp membershipをexplicit Applyの追加instanceへ複製し、通常derived経路でconnection outputを再構築する | `anchor` `differential` `presence` | branch membership推測、random専用pairing、sharp/cross continuity欠落を防ぐ |
| C886 | Behavior | Boundary | 同じseedの別variationは同じplacement keyを持てるが、Bundle identity、descriptor、Apply scopeは完全に独立する | `anchor` `differential` | placement keyをrepo全体のBundle identityとして扱い、別routeを誤更新するfalse collisionを防ぐ |
| C887 | Behavior | Boundary | 直前deployのv4 variation archiveはsingle/multiple/zero instanceのいずれもrepresentative placementを推測せず、legacy node/edge snapshotをSavedBackboneGraphのretained edge参照へ正規化して安定再保存・0 -> 1復元できる | `presence` `anchor` `differential` | 同じwire_state_v4内のfield変更で旧workspaceをload不能にする、同template/count複数instanceから任意の1本をreplay authorityに選ぶ、zero-state topologyを失う回帰を防ぐ |
| C888 | Behavior | Boundary | variation branch pickはvariation IDからexact live Bundle scopeを解決し、0-instance descriptor ruleをphysical branch modeへ含めない | `presence` `anchor` | Webがrecipe rulesから物理scopeを再構成し、存在しないHV等のpolicyで正当なbranch pickを拒否する回帰を防ぐ |
| C889 | Behavior | Boundary | variation membershipは現在descriptorのeffective groupだけを保持し、同一skeletonを複数variationが参照する間は保持し、最後の参照解放でownerless edge/nodeを回収する | `anchor` `differential` | 過去groupの履歴membershipがlatent topologyを永久保持する、duplicate groupがreplay ownerを曖昧にする、共有skeletonを早期回収する回帰を防ぐ |
| C890 | Behavior | Boundary | Bundleを持たないdormant skeletonはcurrent topology query・context・source・pass-through判断から隔離し、同一node pairの再利用は保存identityと幾何がexact compatibleな場合だけ許す | `anchor` `differential` | latent edgeがlive topologyへ混入する、dormant metadataが通常生成を変える、または同一node pairの不整合edgeを黙って再利用する回帰を防ぐ |
| C891 | Behavior | Invariant | 同一live topologyへ同じ通常extensionを行うとき、接続点にincidentなdormant edgeとdormant nodeのbundle mode/source metadataの有無で、Bundle relation・pairing・pass-through・branch/junction・source projection・public topologyを変えない | `differential` `anchor` | dormant skeletonがcontext edgeやsource候補として通常generationへ再混入する回帰を防ぐ |
| C892 | SourceGuard | Boundary | generic pipelineとloaded-output rebuildはvariation membershipをscanせず、live topologyをEdgeBundle relationだけから導出する | `source_guard` | variation固有retentionがpipeline stageへ戻る、またはload/rebuildがmembership edgeをlive route/contextへ混入する回帰を防ぐ |
| C893 | Behavior | Boundary | 同一physical node pairとgeometryのedgeは、後続route内のroute/orderが変わってもlive/dormantで同じcompatibility判定を使い、edge identityを維持しつつ新しいEdgeBundleだけが現在route/orderを持つ | `differential` `anchor` | 初回route-local metadataをphysical edge identityとして扱い、dormant時だけ合法なsame-pair reuseを拒否する回帰を防ぐ |
| C894 | Behavior | Boundary | 現行sharp row契約はcategoryによらずcornerに2 Portを置きJumperで接続する。Communicationの最終viewer sampleで各Jumper endpointは対応するmain body endpointと一致し、両curveのendpoint-away tangentは逆向き同一直線となる | `anchor` `oracle` | 非HVだけ未定義にpoint-through化すること、短い接続線がmain bodyへ非G1で接続する回帰を防ぐ |
