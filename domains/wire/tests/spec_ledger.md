...
| C896 | Behavior | Invariant | Coreはsag済みVisualCurvePartとhelix axis/resolved radiusだけを決定し、asset catalogを知らずにrebuild/save-loadで同じcurve semanticsを維持する | `oracle` `differential` `presence` | asset presentation decisionのCore逆流、helix geometry semanticの欠落、GLB導入によるlogical topology変化を防ぐ |
