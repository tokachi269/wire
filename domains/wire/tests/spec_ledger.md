...
| C881 | Behavior | Boundary | duplicate/nonzero key、scope外key衝突、template/layer/count/spacing、current-scope外anchor、anchor lane topology不一致をmutation前またはouter trial内で拒否する | `differential` `anchor` | category/template/geometryによるscopeやanchor推測、silent normalization、暗黙template migrationを防ぐ |
