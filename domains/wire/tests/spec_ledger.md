...
| C882 | Behavior | Invariant | random route生成はexplicit-Apply descriptor、exact Bundle scope、将来のmembership sourceをconcrete topologyと同じtransactionで保存し、save-load-saveでrerollせず同一identityを維持する | `anchor` `differential` `presence` | session-only recipe、descriptorだけの部分保存、load時reroll、scope identity欠落を防ぐ |
...
| C884 | Behavior | Invariant | 1 -> 0 -> 1のexplicit Applyはhidden Bundleを残さず、保存したphysical membershipとrow continuityをconstraintとして同じplacement identityへ再materializeする | `anchor` `differential` `presence` | 0件時のmembership喪失、category/geometryからのroute推測、pairing再判断を防ぐ |
