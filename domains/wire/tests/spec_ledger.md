...
| C884 | Behavior | Boundary | 1 -> 0はBundleと物理topologyを退役し、live exact sourceのない0 -> 1はstate不変のままunsupportedにする | `anchor` `differential` | v1対象外のdormant skeleton/replayを残す、または削除済みmembershipを推測して復活させる回帰を防ぐ |
...
| C892 | SourceGuard | Boundary | generic pipelineとloaded-output rebuildはvariation retentionをscanせず、live topologyをEdgeBundle relationだけから導出する | `source_guard` | variation固有retentionやdormant replayがpipeline/loadへ戻る回帰を防ぐ |
...
| C864 | Behavior | Invariant | 非HV logical centerlineは通常のsag curveを維持する。3-member visual bundleは線径由来のcompactな三角断面を全長とendpointで保ち、NodePatchを含む接続曲線にも同じ固定断面を引き継ぐ。非HV sagだけが明示spread内で変化する | `anchor` `oracle` `differential` | random wander/phase/twistの復活、endpointで一点へ収束する扇形、接続部だけcenter 1本へ戻る回帰、横一列、member衝突・離脱、HV形状変化を防ぐ |
...
| C896 | Behavior | Invariant | Coreはsag済みVisualCurvePartを4m main / 1m connectionのappearance pieceで隙間なく覆い、asset variant・180度反転・helix familyをsource identityから決定してrebuild/save-loadでも維持する | `oracle` `differential` `presence` | Webがpiece数・variant・sagを再決定する、source assetを伸長する、またはGLB導入でlogical topologyを変える回帰を防ぐ |
