...
using city::wire::AttachmentLineInteractionMode;
using city::wire::BundleKind;
using city::wire::ChangeSet;
using city::wire::ConnectionCategory;
using city::wire::CoreState;
using city::wire::CoreStateTestHook;
using city::wire::kDefaultHighVoltageBundleTemplateId;
using city::wire::ObjectId;
using city::wire::PlacementMode;
using city::wire::PoleKind;
using city::wire::Transformd;
using city::wire::Vec3d;

bool has_validation_issue(const city::wire::ValidationResult& validation, city::wire::ValidationSeverity severity,
                         const char* code) {
  return std::any_of(validation.issues.begin(), validation.issues.end(),
                     [&](const city::wire::ValidationIssue& issue) {
                       return issue.severity == severity && issue.code == code;
                     });
}

double template_layer_base_z_for_test(const city::wire::CoreState& state, ObjectId pole_id,
                                      city::wire::ConnectionCategory category) {
  const auto& view = state.view();
  const city::wire::Pole* pole = view.edit_state().poles.find(pole_id);
  if (pole == nullptr) {
    return 0.0;
  }
  ...
  const int target_layer = city::wire::generation::detail::TemplateLayerForCategory(category);
...
const city::wire::OverrideEntryView* find_override_entry(const city::wire::OverrideInspectionView& view,
                                                         const char* name) {
  const auto it = std::find_if(view.entries.begin(), view.entries.end(),
                               [&](const city::wire::OverrideEntryView& entry) { return entry.name == name; });
  return it == view.entries.end() ? nullptr : &(*it);
}

std::vector<ObjectId> collect_ids_from_ports(const std::vector<const city::wire::Port*>& ports) {
...
std::vector<ObjectId> collect_ids_from_anchors(const std::vector<const city::wire::Anchor*>& anchors) {
  std::vector<ObjectId> ids;
...
bool prepare_single_low_voltage_span(CoreState& state, ObjectId* span_id, city::wire::CableTemplateId* cable_template_id) {
  const auto fixture = helpers::make_backbone_fixture(
      state, {{0.0, 0.0, 0.0}, {12.0, 0.0, 0.0}}, {city::wire::BundleKind::kLowVoltage});
  if (!fixture.ok || fixture.value.spans.empty()) return false;
  if (!state.ValidateFast().ok()) {
    return false;
  }
  const city::wire::Span* span = state.view().spans().find(fixture.value.spans.front());
  if (span == nullptr) {
    return false;
  }
...
bool test_update_bundle_template_regenerates_branch_down_offset_policy() {
  ...
  if (!fixture.ok || fixture.value.spans.empty()) return false;
  const city::wire::Span* hv_span = state.view().spans().find(fixture.value.spans.front());
  if (hv_span == nullptr) {
    return false;
  }
  const ObjectId hv_span_id = hv_span->id;
  const city::wire::Bundle* hv_bundle = state.view().bundles().find(hv_span->bundle_id);
  const city::wire::BundleTemplate* hv_template =
  ...
  city::wire::BundleTemplate edited = *hv_template;
...
bool test_update_cable_template_rejects_manual_span_render_change() {
  ...
  city::wire::CableTemplateId cable_id = city::wire::kInvalidCableTemplateId;
...
bool test_update_cable_template_decision_change_regenerates_backbone() {
  ...
  city::wire::CableTemplate edited = cable_it->second;
  edited.continuity_policy = (edited.continuity_policy == city::wire::CableContinuityPolicyHint::kPreferG1)
