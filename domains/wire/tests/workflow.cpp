...
namespace {
city::wire::BackboneSpec simple_line_spec(const city::wire::BackboneInputSpec& path, double interval,
...
bool test_add_connection_same_pole_fails_and_recovers() {
  ...
  city::wire::Transformd pole_b_tf{};
...
bool test_style_context_resolver_is_deterministic_and_route_correlated() {
  city::wire::ContextProfile profile{};
  profile.age = 0.7;
  ...
  city::wire::StyleRouteKey route_key{};
  route_key.family_id = 1001;
  route_key.bundle_template_id = city::wire::DefaultBundleTemplateId(city::wire::BundleKind::kCommunication);
  route_key.category = city::wire::ConnectionCategory::kCommunication;
  route_key.flow_kind = city::wire::BackboneFlowKind::kMain;

  city::wire::StyleObjectKey object_a{};
  object_a.route = route_key;
  object_a.segment_index = 3;
  object_a.lane_index = 0;
  object_a.kind = city::wire::StyleObjectKind::kSpan;
  object_a.ordinal = 0;

  city::wire::StyleObjectKey object_b = object_a;
  object_b.ordinal = 1;

  const city::wire::ResolvedStyleContext resolved_a =
      city::wire::ResolveStyleContext(profile, route_key, object_a);
  const city::wire::ResolvedStyleContext resolved_a_repeat =
      city::wire::ResolveStyleContext(profile, route_key, object_a);
  const city::wire::ResolvedStyleContext resolved_b =
      city::wire::ResolveStyleContext(profile, route_key, object_b);
