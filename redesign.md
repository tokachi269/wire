...
* `interval_m` と単一 segment avoid detour が同じ segment にある場合、interval point と detour point は source segment 上の `t` 順で route に挿入する。
...
* simple avoid detour: route の単一 segment 上に単一 positive avoid point がある場合、bb2 は route 上流で detour node を挿入する。
