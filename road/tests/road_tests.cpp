...
using wire::road::ErrorKind;
using wire::road::JapaneseUrbanTwoLaneTemplate;
using wire::road::MakeBezier;
using wire::road::MakeLine;
using wire::road::MakePath;
using wire::road::ManualAreaMarking;
using wire::road::ManualLineMarking;
using wire::road::PreviewRoadToolPath;
using wire::road::RoadState;
using wire::road::SectionTransition;
using wire::road::SectionTransitionRule;
using wire::road::StationRef;
using wire::road::StationRefKind;
using wire::road::ThreeLaneTemplate;
using wire::road::TransitionAction;
using wire::road::ValidateGraphInvariants;
using wire::road::Vec2d;
...
bool road_does_not_enter_wire_core(std::string& failure) {
  ...
  for (const auto& entry : std::filesystem::recursive_directory_iterator(core)) {
    ...
    ROAD_TEST_EXPECT(text.find("wire/road") == std::string::npos,
